...
public class VersionedHandleIdentifierProvider extends IdentifierProvider {
    ...
    public void register(Context context, DSpaceObject dso, String identifier)
    {
        try
        {
            ...
            {
                //We need to have a version history to be able to do this !
                Item item = (Item) dso;
                VersionHistory history = retrieveVersionHistory(context, item);
                if(history != null)
                {
                    TableRow handleRecord = findHandleInternal(context, identifier);
                    if(handleRecord != null)
                    {
                        modifyHandleRecord(context, dso, handleRecord, identifier);
                    }else{
                        createNewIdentifier(context, dso, identifier);
                        populateHandleMetadata(item);
                    }
                } else {
                    throw new IllegalArgumentException("You can only use identifiers with the following format: {handle.prefix}/{handle}.{version} if a version history is present for the item !");
                }
            }else{
                //A regular handle
                createNewIdentifier(context, dso, identifier);
