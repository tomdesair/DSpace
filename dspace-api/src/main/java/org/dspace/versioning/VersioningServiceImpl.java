    ...
    public Version createNewVersion(Context c, int itemId, String summary) {
        try{
            ...
            if(vh==null)
            {
                ...
                createVersion(c, vh, item, "");
            }
            // Create new Item
            Item itemNew = provider.createNewItemAndAddItInWorkspace(c, item);

            // create new version
            Version version = createVersion(c, vh, itemNew, summary);
...
// **** PROTECTED METHODS!!
    ...
    protected VersionImpl createVersion(Context c, VersionHistory vh, Item item, String summary) {
        try {
